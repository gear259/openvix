#!/usr/bin/sh
#Restore the user files stored from backup on the hdd 
...
cp -f -v -r /media/hdd/config /etc/tuxbox & cp -v /media/hdd/inadyn.conf /etc & cp -v /media/hdd/autotimer.xml /etc/enigma2 & cp -v /media/hdd/timers.xml /etc/enigma2 & cp -v /media/hdd/userbouquet.favourites.tv /etc/enigma2 & cp -v /media/hdd/fstab /etc && cp -v /media/hdd/interfaces /etc/network & cp -v /media/hdd/wpa_supplicant* /etc & cp -v /media/hdd/hostname /etc &&
